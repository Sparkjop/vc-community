...
IF /I "%APPSETTING_VirtoCommerce_DeployApplications%" NEQ "Manager Only" (
    ...
    call :ExecuteCmd "%MSBUILD_PATH%" "%STORE_SOLUTION_FILE%" /nologo /verbosity:m /t:Build /p:Configuration=SpaDev;Platform="Any CPU";DebugType=none;AllowedReferenceRelatedFileExtensions=":";SolutionDir="%STORE_SOLUTION_DIR%\.\\";OutputPath="%DEPLOYMENT_TEMP%" %SCM_BUILD_ARGS%
    IF !ERRORLEVEL! NEQ 0 goto error
    ...
    :: Clear build output
    call :ExecuteCmd "%MSBUILD_PATH%" "%STORE_SOLUTION_FILE%" /nologo /verbosity:m /t:Clean /p:Configuration=SpaDev;Platform="Any CPU";SolutionDir="%STORE_SOLUTION_DIR%\.\\" %SCM_BUILD_ARGS%
)
...
IF /I "%APPSETTING_VirtoCommerce_DeployApplications%" NEQ "Storefront Only" (
    ...
    call :ExecuteCmd "%MSBUILD_PATH%" "%ADMIN_SOLUTION_FILE%" /nologo /verbosity:m /t:Build /p:Configuration=SpaDev;Platform="Any CPU";DebugType=none;AllowedReferenceRelatedFileExtensions=".xml";SolutionDir="%ADMIN_SOLUTION_DIR%\.\\";OutputPath="%DEPLOYMENT_TEMP%";VCModulesOutputDir="%PUBLISHED_MODULES%" %SCM_BUILD_ARGS%
    IF !ERRORLEVEL! NEQ 0 goto error
    ...
    :: Clear build output
    call :ExecuteCmd "%MSBUILD_PATH%" "%ADMIN_SOLUTION_FILE%" /nologo /verbosity:m /t:Clean /p:Configuration=SpaDev;Platform="Any CPU";SolutionDir="%ADMIN_SOLUTION_DIR%\.\\" %SCM_BUILD_ARGS%
)
